package me.expresshighway.jwttutorial.jwt;
...
public class JwtFilter extends GenericFilterBean {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain)
        ...
        String jwt = resolveToken(httpServletRequest); // 토큰을 받아오기
        String requestURI = httpServletRequest.getRequestURI();

        if (StringUtils.hasText(jwt) && tokenProvider.validateToken(jwt)) { //유효성 검증
            Authentication authentication = tokenProvider.getAuthentication(jwt); // 정상 토큰이면
